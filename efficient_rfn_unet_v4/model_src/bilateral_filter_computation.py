...
class BilateralHighDimFilterComputation(tf.Module):
    @tf.function(input_signature=[
        tf.TensorSpec(shape=[None, None, None], dtype=tf.float32), # of `features`, [H, W, C], float32
    ...
    def init(self, features: tf.Tensor, dim: tf.int32, range_sigma: tf.float32, space_sigma: tf.float32, range_padding: tf.int32, space_padding: tf.int32) -> tf.Tensor:
        """
        ...
        def create_range_params(ch):
            ...
            splat_chh = tf.cast(chh / range_sigma + .5, dtype=tf.int32) + range_padding
        ...
        splat_yy = tf.cast(yy / space_sigma + .5, dtype=tf.int32) + space_padding
        splat_xx = tf.cast(xx / space_sigma + .5, dtype=tf.int32) + space_padding
        ...
        permutations = tf.reshape(permutations, [-1, dim]) # [2^d, d]
        permutations += offset[tf.newaxis, ...]
        permutations = tf.reshape(permutations, shape=[-1, ]) # flatten, [2^d x d, ]
        alpha_prods = tf.reshape(tf.gather(alphas, permutations), [-1, dim, size]) # [2^d, d, H x W]
        idx = tf.reshape(tf.gather(interp_indices, permutations), [-1, dim, size]) # [2^d, d, H x W]
    ...
    @tf.function(input_signature=[
        ...
        tf.TensorSpec(shape=[None, ], dtype=tf.float32), # of `alpha_prod`, [H x W, ], float32
        tf.TensorSpec(shape=[], dtype=tf.int32), # of `n_iters`, [], int32
    ])
    def compute(inp: tf.Tensor, dim: tf.int32, splat_coords: tf.Tensor, data_size: tf.int32, data_shape: tf.Tensor, slice_idx: tf.Tensor, alpha_prod: tf.Tensor, n_iters: tf.int32) -> tf.Tensor:
