...
class BilateralHighDimFilterComputation(tf.Module):
    def __init__(self, height, width, n_channels, range_sigma, space_sigma, range_padding=2, space_padding=2, n_iters=2, name=None):
        ...
        self.height, self.width, self.n_channels = tf.constant(height, dtype=tf.int32), tf.constant(width, dtype=tf.int32), tf.constant(n_channels, dtype=tf.int32)
        self.size = self.height * self.width
        self.dim = self.n_channels + 2

        self.range_sigma, self.space_sigma = tf.constant(range_sigma, dtype=tf.float32), tf.constant(space_sigma, dtype=tf.float32)

        self.range_padding, self.space_padding = tf.constant(range_padding, dtype=tf.int32), tf.constant(space_padding, dtype=tf.int32)
